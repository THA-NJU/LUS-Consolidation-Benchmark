#!/usr/bin/env python3
"""Evaluate the three Size_224 models from the supplied APRIL training script.

Only these models are supported:
  * ../Pth/CNN/224/mednext
  * ../Pth/Transformer/224/pvtb2_emcad
  * ../Pth/other/224/rolling_unet

The model definitions and inference preprocessing follow the supplied Size_224
training script: native 224x224 inputs, three-channel replicated grayscale,
fixed [0, 1] input scaling, two output classes, and BF16 autocast by default.

Run this script from the APRIL-MedSeg repository in the original environment,
and keep unified_native_segmentation_eval.py beside this file.
"""
...
DEFAULT_OUTPUT_ROOT = Path("../Evaluation/mednext_pvtb2_rolling_224")
...
def build_model(
    ...
    # The APRIL snapshot used for training contains the implementation but may
    # omit PVTB2-EMCAD from its _SPECIAL_ARCHS registry.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Size_224_filtered root containing train/val/test.",
    )
    parser.add_argument(
        "--models",
        nargs="+",
    ...
    parser.add_argument(
        ...
        default="auto",
        help="auto uses BF16, matching the supplied Size_224 training script.",
    )
    parser.add_argument(
        "--no-auto-batch",
        action="store_true",
...
def main() -> None:
    ...
    if not model_names:
        raise FileNotFoundError(
            "None of the three supported Size_224 model directories was found"
        )

    print("=" * 88)
    print("MedNeXt / PVTB2-EMCAD / Rolling U-Net Size_224 evaluation")
