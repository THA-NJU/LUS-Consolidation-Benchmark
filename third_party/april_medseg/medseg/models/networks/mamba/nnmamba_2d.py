...
class _MambaSSM(nn.Module):
    ...
    def forward(self, x):
        # The long selective-state recurrence is more numerically fragile than
        # the surrounding CNN blocks under FP16/BF16 autocast. Run the Mamba
        # core in FP32, then cast back so the rest of the network can still use
        # mixed precision. Gradients propagate through both casts.
        original_dtype = x.dtype
        device_type = x.device.type
        with torch.autocast(device_type=device_type, enabled=False):
            y = self.mamba(x.float())
