#!/usr/bin/env python3
"""Single-GPU Size_512 Mamba/SSM consolidation benchmark for APRIL-MedSeg.

This script deliberately uses APRIL-MedSeg's model registry/build_model while
providing a dedicated dataset, training loop, fixed-threshold evaluation, and binary
metrics for the user's existing explicit train/val/test directory structure.
It does not modify the original train.py/test.py.

Workflow:
  1. Edit the USER SETTINGS section below.
  2. Run check_consolidation_size512.py first.
  3. Keep RUN_MODE='smoke' for a short pipeline test.
  4. Change RUN_MODE='formal' and run this file again.

Outputs per architecture:
  best_model.pth, last_model.pth, history.csv,
  test_cases.csv, result.json, and an aggregate benchmark_summary.csv.
"""
...
OUTPUT_ROOT = PROJECT_ROOT / "output" / f"consolidation_size512_mamba_benchmark_{time.strftime('%Y%m%d_%H%M%S')}"

GPU_ID = 0
RUN_MODE = "formal"  # 'smoke' first; change to 'formal' after it succeeds
MODELS_TO_RUN = [
    "swin_umamba",
    "mamba_unet",      # Mamba-UNet
    "vm_unet_v2",     # VM-UNet V2 (the APRIL registry has no BM-UNet V2)
    "nnmamba_2d",     # 2D adaptation of nn-Mamba
    "dcm_net",        # dual-encoder CNN-Mamba, ultrasound-oriented
]

IMAGE_SIZE = 512
NUM_CLASSES = 2  # 0 background, 1 consolidation
IN_CHANNELS = 3  # grayscale is replicated to three channels
PRETRAINED = False  # fair architecture comparison: train every model from scratch
SEED = 42
DETERMINISTIC = False
USE_AMP = True
NUM_WORKERS = 4  # set 0 if Windows multiprocessing causes problems
PIN_MEMORY = True
...
# Numerical-stability safeguards. Mamba-family models are more sensitive to
# fp16 overflow because the selective-scan recurrence contains exponentials and
# long state propagation. The benchmark therefore supports per-model precision,
# gradient clipping, warmup, and non-finite batch detection.
DEFAULT_AMP_DTYPE = "fp16"  # fp16 | bf16 | fp32
...
CHECKPOINT_SELECTION_METRIC = "dice_positive"  # avoids empty-negative inflation
...
# Smoke run tests the entire pipeline quickly. Formal mode uses a fixed image
# budget per epoch, so all architectures see the same amount of data and have
# the same effective batch size despite different physical batch sizes.
RUN_PRESETS = {
    "smoke": {
        "epochs": 1,
...
# Architecture-specific memory settings. All enabled defaults use an effective
# batch size of 4 through gradient accumulation.
MODEL_SPECS = {
    # The four models use the same optimizer family, LR, warmup, physical batch,
    # effective batch, precision policy, and stability safeguards. This makes the
    # within-family benchmark easier to interpret. Architecture parameters follow
    # APRIL's supplied general-network YAMLs or the constructors' defaults.
    "mamba_unet": {
...
def dependency_preflight(model_name: str) -> None:
    mamba_models = {"mamba_unet", "vm_unet_v2", "nnmamba_2d", "dcm_net"}
    if model_name in mamba_models:
        if not torch.cuda.is_available():
            raise RuntimeError(f"{model_name} requires a CUDA GPU at Size_512")
        try:
            import selective_scan_cuda  # noqa: F401
            from mamba_ssm.ops.selective_scan_interface import selective_scan_fn  # noqa: F401
        except Exception as exc:
            raise RuntimeError(
                f"{model_name} requires the CUDA-enabled mamba_ssm build and "
                "selective_scan_cuda. Verify the april_ssm environment before training."
            ) from exc
...
def build_april_model(model_name: str) -> Tuple[nn.Module, Dict[str, object]]:
    ...
    # The uploaded APRIL-MedSeg snapshot contains pvtb2_emcad_model.py and a
    # YAML config, but omits this architecture from networks._SPECIAL_ARCHS.
    # Register it at runtime so the benchmark can use the intended complete
    # architecture without editing the upstream package.
    if model_name == "pvtb2_emcad":
        from medseg.models import networks
        from medseg.models.networks.transformer.pvtb2_emcad_model import PVTB2EMCAD
    ...
    for step, batch in enumerate(progress):
        ...
        if should_step:
            ...
            if not torch.isfinite(grad_norm):
                ...
                print(
                    f"[WARN] epoch={epoch} step={step}: "
                    f"non-finite gradient; update skipped "
                    f"({skipped_steps} skipped in this epoch)"
                )

                if skipped_steps >= 20:
                    raise FloatingPointError(
                        f"Too many non-finite gradients in epoch={epoch}: "
                        f"skipped_steps={skipped_steps}. "
                        "Abort this model to avoid training with unstable numerics."
                    )
...
def build_dataloaders(
    ...
    sampler = build_train_sampler(train_ds, OUTPUT_ROOT, preset["samples_per_epoch"])
...
def train_and_evaluate_one(model_name: str, device: torch.device, preset: Dict[str, object]) -> Dict[str, object]:
    ...
    print("\n" + "=" * 88)
    print(f"MODEL: {model_name} | mode={RUN_MODE} | device={device}")
    print("=" * 88)
    ...
    print(
        f"FINAL {model_name}: threshold={FIXED_VALIDATION_THRESHOLD:.2f}, "
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("A CUDA GPU is required for the Size_512 benchmark")
    if GPU_ID < 0 or GPU_ID >= torch.cuda.device_count():
        raise ValueError(
            f"GPU_ID={GPU_ID} is invalid; visible CUDA devices={torch.cuda.device_count()}"
        )
    ...
    print(
        ...
        f"val_interval={RUN_PRESETS['formal']['val_interval']}"
    )
    ...
    for model_name in MODELS_TO_RUN:
        try:
            ...
            print(f"\n[FAILED] {model_name}: {message}")
            traceback.print_exc()
            failures.append({"model": model_name, "error": message})
            save_json(OUTPUT_ROOT / model_name / RUN_MODE / "failure.json", failures[-1])
            if FAIL_FAST:
                raise
        finally:
            gc.collect()
    ...
    summary_path = write_aggregate_summary(results) if results else None
    save_json(
        OUTPUT_ROOT / f"run_report_{RUN_MODE}.json",
        {"results": results, "failures": failures, "summary_csv": str(summary_path) if summary_path else None},
    )
    print("\n" + "=" * 88)
    print(f"Completed models: {len(results)}; failed models: {len(failures)}")
    if summary_path:
