#!/usr/bin/env python3
"""AutoDL A800 benchmark for APRIL-MedSeg architectures at 224x224.
...
This extended version also integrates the following APRIL architectures into
the same Size_224 benchmark loop:
  mednext        : MedNeXt, MICCAI 2023, APRIL 2D adaptation
  pvtb2_emcad    : PVTv2-B2 + EMCAD; runtime-registered for APRIL snapshots
                     where the implementation exists but the registry omits it
  rolling_unet   : Rolling-Unet
  rwkv_unet      : RWKV-UNet, variant B
  u_rwkv         : U-RWKV

RWKV models use APRIL's WKV CUDA extension and therefore require CUDA toolkit
(NVCC) + Ninja in addition to the ordinary APRIL environment. For Size_224,
the WKV kernels use resolution-specific compile ceilings: Tmax=1024 for
RWKV-UNet and Tmax=16384 for U-RWKV; the much larger Size_512 ceilings must
not be reused here.
...
OUTPUT_BASE = PROJECT_ROOT / "output" / f"autodl_easy_models_benchmark_224_{RUN_ID}"
...
MODELS_TO_RUN = [
    # Newly added models for this A800 Size_224 run.
    # "mednext",          # MedNeXt (MICCAI 2023), APRIL 2D adaptation
    # "pvtb2_emcad",      # PVTv2-B2 encoder + EMCAD decoder
    # "rolling_unet",     # Rolling-Unet
    "rwkv_unet",        # RWKV-UNet, variant B; requires APRIL WKV CUDA kernel
    "u_rwkv",           # U-RWKV; requires APRIL WKV CUDA kernel

    # Previously available models. Uncomment only when you intentionally
    # want to rerun them in the same protocol.
    # "nnunet_2d",
    # "aau_net",
    # "swinunet",
    # "nnformer_2d",
...
BALANCE_POSITIVE_IMAGES = False  # 224 benchmark: traverse the dataset naturally; no pos/neg re-balancing
...
CHECKPOINT_SELECTION_METRIC = "dice_all"  # best epoch = highest mean Dice on 224x224 validation patches
...
MODEL_SPECS = {
    ...
    "mednext": {
        ...
        # APRIL exposes a 2D adaptation of MedNeXt for 2D segmentation.
        "override_by_size": {
            224: {"batch_size": 16, "grad_accum": 1, "lr": 1e-4},
        },
    },
    "pvtb2_emcad": {
        ...
        # Keep effective batch = 16.
        "override_by_size": {
            224: {"batch_size": 256, "grad_accum": 2, "lr": 1e-4},
        },
    },
    "rolling_unet": {
    ...
    "rwkv_unet": {
        ...
        # Same effective batch (16), lower physical batch for the custom WKV path.
        "override_by_size": {
            224: {
                "batch_size": 256,
                "grad_accum": 2,
        ...
        # Size_224: first RWKV stage is 28x28 = 784 tokens.
        # Compile the next safe ceiling instead of reusing the old Size_512 Tmax.
        "kernel_tmax": 1024,
    },
    "u_rwkv": {
        "architecture": "u_rwkv",
        ...
        # U-RWKV applies WKV earlier at a higher token count. For Size_224
        # the first sequence is 112x112=12544, so Tmax=16384 is sufficient.
        "override_by_size": {
            224: {
                "batch_size": 256,
                "grad_accum": 2,
        ...
        # Size_224: U-RWKV applies WKV after the 2x stem:
        # 112x112 = 12544 tokens. 16384 is a safe compile-time ceiling.
        "kernel_tmax": 16384,
    },
}


_RWKV_KERNEL_CURRENT_TMAX: Optional[int] = None
...
def unwrap_primary_output(output) -> torch.Tensor:
    ...
    if isinstance(output, dict):
        ...
        for value in output.values():
            if torch.is_tensor(value):
                return value
        raise RuntimeError(
            f"Model returned a dict without tensor outputs: {list(output.keys())}"
        )
    if isinstance(output, (list, tuple)):
        ...
        for value in output:
            if torch.is_tensor(value):
                return value
        raise RuntimeError("Model output list contains no tensor")
...
def _print_rwkv_build_environment(model_name: str, t_max: int) -> None:
    from torch.utils.cpp_extension import CUDA_HOME

    capability = torch.cuda.get_device_capability(torch.cuda.current_device())
    print("[RWKV BUILD] --------------------------------------------------")
    print(f"[RWKV BUILD] model={model_name}, Size_224 Tmax={t_max}")
    print(f"[RWKV BUILD] torch={torch.__version__}")
    print(f"[RWKV BUILD] torch.version.cuda={torch.version.cuda}")
    print(f"[RWKV BUILD] CUDA_HOME={CUDA_HOME}")
    print(f"[RWKV BUILD] nvcc={shutil.which('nvcc')}")
    print(f"[RWKV BUILD] gcc={shutil.which('gcc')}")
    print(f"[RWKV BUILD] g++={shutil.which('g++')}")
    print(f"[RWKV BUILD] ninja={shutil.which('ninja')}")
    print(
        f"[RWKV BUILD] GPU={torch.cuda.get_device_name(torch.cuda.current_device())}, "
        f"compute_capability={capability[0]}.{capability[1]}"
    )
    print(
        "[RWKV BUILD] nvcc version:\n"
        + _command_output([shutil.which("nvcc") or "nvcc", "--version"])
    )
    print(
        "[RWKV BUILD] g++ version:\n"
        + _command_output([shutil.which("g++") or "g++", "--version"]).splitlines()[0]
    )
    print("[RWKV BUILD] --------------------------------------------------")


def _prepare_rwkv_kernel(model_name: str) -> None:
    """Compile/load APRIL's WKV CUDA extension before an RWKV model is built."""
    if not torch.cuda.is_available():
        raise RuntimeError(f"{model_name} requires a CUDA GPU")

    required = [
        PROJECT_ROOT / "medseg" / "kernels" / "wkv" / "__init__.py",
        PROJECT_ROOT / "medseg" / "kernels" / "wkv" / "wkv_op.cpp",
        PROJECT_ROOT / "medseg" / "kernels" / "wkv" / "wkv_cuda.cu",
        PROJECT_ROOT / "medseg" / "models" / "networks" / "rwkv" / "rwkv_unet.py",
        PROJECT_ROOT / "medseg" / "models" / "networks" / "rwkv" / "u_rwkv.py",
    ]
    missing = [str(p) for p in required if not p.is_file()]
    if missing:
        raise FileNotFoundError(
            "APRIL checkout is missing the unified RWKV implementation:\n  "
            + "\n  ".join(missing)
        )
    ...
    if CUDA_HOME is None or shutil.which("nvcc") is None:
        raise RuntimeError(
            "RWKV WKV requires a local CUDA toolkit/NVCC. "
            "A PyTorch CUDA runtime alone is not sufficient."
        )
    if (
    ...
    global _RWKV_KERNEL_CURRENT_TMAX

    raw_spec = MODEL_SPECS[model_name]
    t_max = int(raw_spec["kernel_tmax"])
    if _RWKV_KERNEL_CURRENT_TMAX == t_max:
        return
    ...
    _print_rwkv_build_environment(model_name, t_max)
    print(
        f"[RWKV] compiling/loading APRIL WKV CUDA kernel: "
        f"model={model_name}, Tmax={t_max}"
    )
    # APRIL's WKV module keeps a process-global loaded operator. Force reload
    # when switching Tmax so rwkv_unet and u_rwkv can safely run sequentially
    # in this same benchmark process.
    op = load_wkv_cuda(
        t_max=t_max,
        force=True,
        verbose=True,
    ...
    if op is None or not is_cuda_available():
        raise RuntimeError(
            "WKV CUDA compilation/load failed. "
            f"model={model_name}, Size_224 Tmax={t_max}, "
            f"torch={torch.__version__}, torch_cuda={torch.version.cuda}, "
            f"CUDA_HOME={CUDA_HOME}, error={get_load_error()!r}. "
            "The useful compiler diagnostic is the nvcc/g++ output printed "
            "immediately above this exception."
        )

    _RWKV_KERNEL_CURRENT_TMAX = t_max
    print(f"[RWKV] WKV CUDA kernel ready: Tmax={t_max}")


def dependency_preflight(model_name: str) -> None:
    """Run only model-specific checks that are needed before build_model()."""
    if model_name in {"rwkv_unet", "u_rwkv"}:
        _prepare_rwkv_kernel(model_name)
        return

    # Mamba/SAM families remain outside this script. They need their own
    # environments/training semantics and are intentionally blocked here.
    excluded_models = {
        "swin_umamba", "mamba_unet", "vm_unet_v2", "nnmamba_2d", "dcm_net",
        "sam2", "sam3", "medsam", "samus", "sam_b", "sam_l", "mobile_sam",
        "sam_med2d", "medical_sam_adapter", "samed",
    }
    if model_name in excluded_models:
        raise RuntimeError(
            f"{model_name} is intentionally excluded from this A800 script."
        )
...
def _register_optional_april_architectures(model_name: str) -> None:
    """Register APRIL architectures that exist in the source tree but are
    omitted from the current snapshot's model-builder registry.

    In the APRIL snapshot used by this benchmark, PVTv2-B2-EMCAD is implemented
    in medseg/models/networks/transformer/pvtb2_emcad_model.py, but
    `pvtb2_emcad` is not present in networks._SPECIAL_ARCHS. Registering it
    here preserves the upstream implementation while avoiding any modification
    of the installed APRIL package.
    """
    ...
    model_file = (
        ...
        / "networks"
        / "transformer"
        / "pvtb2_emcad_model.py"
    )
    if not model_file.is_file():
        raise FileNotFoundError(
            "PVTv2-B2-EMCAD was requested, but the APRIL implementation is "
    ...
    try:
        ...
        raise RuntimeError(
            "APRIL contains pvtb2_emcad_model.py but it could not be imported. "
            "Check the traceback for a missing EMCAD/PVT dependency."
        ) from exc

    special_archs = getattr(networks, "_SPECIAL_ARCHS", None)
    if special_archs is None:
        raise RuntimeError(
            "This APRIL snapshot has no networks._SPECIAL_ARCHS registry; "
            "cannot safely register pvtb2_emcad at runtime."
        )

    if "pvtb2_emcad" not in special_archs:
        special_archs["pvtb2_emcad"] = PVTB2EMCAD
        print(
            "[APRIL] runtime-registered architecture: "
            "pvtb2_emcad -> PVTB2EMCAD"
        )
...
def build_april_model(model_name: str) -> Tuple[nn.Module, Dict[str, object]]:
    ...
    _register_optional_april_architectures(model_name)
...
def make_grad_scaler(enabled: bool, amp_dtype: torch.dtype):
    # FP16 才需要 GradScaler；BF16/FP32 不需要。
...
def autocast_context(enabled: bool, amp_dtype: torch.dtype):
    active = bool(enabled and amp_dtype != torch.float32)

    return torch.cuda.amp.autocast(
        enabled=active,
        dtype=amp_dtype if active else torch.float16,
    )
...
def train_and_evaluate_one(model_name: str, device: torch.device, preset: Dict[str, object]) -> Dict[str, object]:
    ...
    save_json(model_dir / "run_settings.json", {
        ...
        "model_notes": {
            "mednext": (
                "APRIL 2D adaptation of MICCAI 2023 MedNeXt; "
                "the original paper/repository primarily evaluated 3D segmentation."
            ) if model_name == "mednext" else "",
            "rwkv_kernel_tmax": spec.get("kernel_tmax"),
        },
        "numerical_stability": {
...
def write_aggregate_summary(results: Sequence[Dict[str, object]]) -> Path:
    ...
    rows.sort(
        key=lambda r: float(r["test_dice_all"])
        if r.get("test_dice_all") is not None else -1.0,
        reverse=True,
    )
...
def write_global_summary(output_base: Path, results: Sequence[Dict[str, object]], failures: Sequence[Dict[str, str]]) -> Path:
    ...
    rows.sort(
        key=lambda r: (int(r.get("image_size") or 0), float(r.get("test_dice_all") or -1.0)),
        reverse=True,
    )
    write_rows_csv(path, rows)
...
def main() -> None:
    ...
    print("NEW MODEL BATCH: MedNeXt(2D), PVTv2-B2-EMCAD, Rolling-Unet, RWKV-UNet, U-RWKV")
    print(f"GPU_IDS: {GPU_IDS}")
    print(
        "224 BEST-MODEL POLICY: highest validation dice_all at fixed threshold "
        f"{FIXED_VALIDATION_THRESHOLD:.2f}; evaluation is performed directly on 224x224 patches."
    )
    print(
