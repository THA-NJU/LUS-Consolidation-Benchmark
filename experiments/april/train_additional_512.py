#!/usr/bin/env python3
"""Final Size_512 benchmark for APRIL-backed additional architectures.
...
Workflow:
This release entry is dispatched by ``tools/train.py``.  Its formal preset is
the frozen 600-epoch benchmark protocol; the historical 60-epoch preset is
kept only under ``archive/obsolete_protocol``.
...
OUTPUT_ROOT = PROJECT_ROOT / "output" / "consolidation_size512_benchmark"

GPU_ID = 0
RUN_MODE = "formal"  # 'smoke' first; change to 'formal' after it succeeds
MODELS_TO_RUN = [
    # "unet",             # same-protocol control baseline
    # "aau_net",          # ultrasound-oriented attention CNN
    # "mednext",          # modern ConvNeXt-style medical CNN
    # "pvtb2_emcad",      # hierarchical Transformer + EMCAD
    # "rolling_unet",     # MLP/token-mixing family
    "mednext",
    # "u_rwkv",         # optional; requires a working CUDA/NVCC toolchain for 512
]
...
CHECKPOINT_SELECTION_METRIC = "dice_all"
...
# Smoke run tests the entire pipeline quickly. Formal mode traverses the full
# training set once per epoch without a replacement sampler.
RUN_PRESETS = {
    "smoke": {
        "epochs": 1,
...
MODEL_SPECS = {
    ...
    "mednext": {
        ...
        "arch_params": {"model_id": "S", "kernel_size": 3},
        "batch_size": 4,
        "grad_accum": 1,
        "lr": 1e-4,
    },
    "pvtb2_emcad": {
        ...
        "arch_params": {"deep_supervision": False},
        "batch_size": 2,
        "grad_accum": 2,
        "lr": 1e-4,
    },
...
def dependency_preflight(model_name: str) -> None:
    if model_name == "swin_umamba":
        try:
            from mamba_ssm.ops.selective_scan_interface import selective_scan_fn  # noqa: F401
        except Exception as exc:
            raise RuntimeError(
                "swin_umamba requires mamba-ssm. Install a CUDA-compatible build "
                "before enabling it: pip install causal-conv1d && pip install mamba-ssm"
            ) from exc
    if model_name == "u_rwkv":
        if not torch.cuda.is_available():
            raise RuntimeError("u_rwkv at 512 is not practical without CUDA")
        from torch.utils.cpp_extension import CUDA_HOME
        if CUDA_HOME is None:
            raise RuntimeError(
                "u_rwkv needs a working CUDA toolkit/NVCC to compile its WKV kernel. "
                "CUDA runtime alone is not sufficient."
            )
...
def train_one_epoch(
    ...
    for step, batch in enumerate(progress):
        ...
        with autocast_context(USE_AMP and device.type == "cuda"):
...
def train_and_evaluate_one(model_name: str, device: torch.device, preset: Dict[str, object]) -> Dict[str, object]:
    ...
    history_fields = [
        "epoch", "train_loss", "lr", "seconds", "threshold",
