#!/usr/bin/env python3
"""SAMUS ViT-B benchmark for binary lung-ultrasound consolidation.

The script is intended to sit in the APRIL project root. It uses the official
SAMUS source tree with the official SAM ViT-B initialization, while retaining
the Size_512 benchmark protocol used for SAM2 and MedSAM:
...
* every image receives the same full-image box [0, 0, 256, 256];
...
SAMUS natively uses a 256x256 encoder input. Its official data pipeline converts
ultrasound images to tensors in [0, 1] without ImageNet/SAM normalization; the
same preprocessing is used here. The default ``native`` scope reproduces the
official SAMUS freezing policy: only the ultrasound adapters/CNN branch,
post-position layers, selected relative-position parameters, and up-neck layers
are trainable. Prompt and mask decoders remain frozen.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Official Segment Anything ViT-B initialization used by SAMUS.",
    )
    parser.add_argument(
        "--samus-source-dir",
        type=Path,
        default=Path("~/third_party/SAMUS"),
        help="Official xianlin7/SAMUS checkout.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        "--train-scope",
        choices=("native", "native_decoder", "full"),
        default="native",
        help=(
            "native reproduces official SAMUS adapter tuning; native_decoder "
            "also trains the mask decoder; full unfreezes the complete model."
        ),
    )
    parser.add_argument(
        "--amp-dtype",
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.model_input_size != 256:
        raise ValueError("SAMUS ViT-B requires --model-input-size 256.")
...
def resolve_pairs(split_root: Path) -> list[tuple[Path, Path]]:
    ...
    for image_path in images:
        ...
        if mask_path is None:
            candidates = [
                *masks_by_stem.get(image_path.stem, []),
                *masks_by_stem.get(f"{image_path.stem}_mask", []),
            ]
...
class ConsolidationDataset(Dataset[dict[str, Any]]):
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        # Official SAMUS uses torchvision.to_tensor on grayscale ultrasound.
        # Its image encoder repeats one channel to RGB internally.
        return {"image": image, "mask": mask, "name": image_path.name}


def import_samus_registry(source_dir: Path):
    models_dir = source_dir / "models"
    build_file = models_dir / "segment_anything_samus" / "build_sam_us.py"
    if not build_file.is_file():
        raise FileNotFoundError(f"SAMUS source is incomplete: {build_file}")

    # Import the SAMUS package directly from its models directory. This avoids
    # colliding with an unrelated top-level ``models`` package inside APRIL.
    ...
    if models_dir not in imported_path.parents:
        raise RuntimeError(
            f"segment_anything_samus was imported from {imported_path}, "
            f"not {models_dir}."
        )
    return samus_model_registry, imported_path


class SAMUSFixedFullBox(nn.Module):
    """Differentiable SAMUS forward with one identical, non-GT box."""

    def __init__(self, samus_model: nn.Module, model_input_size: int = 256) -> None:
        super().__init__()
        self.samus = samus_model
        self.model_input_size = model_input_size
        self.train_prompt_encoder = False

    def forward(self, images: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        image_embeddings = self.samus.image_encoder(images)
        batch_size = images.shape[0]
        edge = float(self.model_input_size)
        boxes = images.new_tensor([0.0, 0.0, edge, edge])
        boxes = boxes.view(1, 4).expand(batch_size, -1)

        prompt_context = nullcontext() if self.train_prompt_encoder else torch.no_grad()
        with prompt_context:
            sparse_embeddings, dense_embeddings = self.samus.prompt_encoder(
                points=None, boxes=boxes, masks=None
            )
        low_res_logits, iou_predictions = self.samus.mask_decoder(
            image_embeddings=image_embeddings,
            image_pe=self.samus.prompt_encoder.get_dense_pe(),
...
def configure_trainable_parameters(
    model: SAMUSFixedFullBox, scope: str
) -> list[str]:
    ...
    if scope == "full":
        ...
        model.train_prompt_encoder = True
        return [name for name, parameter in model.named_parameters() if parameter.requires_grad]

    model.train_prompt_encoder = False
    for name, parameter in model.samus.image_encoder.named_parameters():
        if any(token in name for token in NATIVE_SAMUS_NAME_TOKENS):
            parameter.requires_grad = True
    if scope == "native_decoder":
        for parameter in model.samus.mask_decoder.parameters():
            parameter.requires_grad = True
    ...
    if not trainable_names:
        raise RuntimeError(
            "No native SAMUS parameters were selected. The official source "
            "layout may have changed; stop rather than training an empty model."
        )
    return trainable_names


def set_training_mode(model: SAMUSFixedFullBox, scope: str) -> None:
    model.train()
    if scope != "full":
        model.samus.prompt_encoder.eval()
    if scope == "native":
        model.samus.mask_decoder.eval()
...
def soft_dice_loss(
    ...
    """Soft Dice loss with squared probabilities."""
...
def compute_loss(
    ...
    dice = soft_dice_loss(logits, target_native)
...
def checkpoint_payload(
    ...
    return {
        # Raw SAMUS state keys preserve compatibility with the official model.
        "model": model.samus.state_dict(),
...
def main() -> None:
    ...
    args.samus_source_dir = args.samus_source_dir.expanduser().resolve()
    if args.resume is not None:
        args.resume = args.resume.expanduser().resolve()
    if not args.checkpoint.is_file():
        raise FileNotFoundError(f"Official SAM ViT-B checkpoint not found: {args.checkpoint}")
    ...
    if args.output_dir is None:
        if args.resume is not None:
            ...
            output_dir = Path(f"benchmark_samus_consolidation_size512_{timestamp}")
    else:
    ...
    print("Initialization: official SAM ViT-B checkpoint used by SAMUS")
    print("Prompt protocol: fixed full-image box; no GT-derived prompt")
    print("Preprocessing: grayscale resize256 + to_tensor [0,1]")
    ...
    registry, imported_path = import_samus_registry(args.samus_source_dir)
    print(f"SAMUS source import: {imported_path}")
    builder_args = SimpleNamespace(encoder_input_size=args.model_input_size)
    samus_model = registry["vit_b"](
        args=builder_args, checkpoint=str(args.checkpoint)
    )
    model = SAMUSFixedFullBox(samus_model, args.model_input_size).to(device)
    trainable_names = configure_trainable_parameters(model, args.train_scope)
    ...
    if args.train_scope == "full" and args.lr >= 1e-4:
        print(
            "WARNING: full-model tuning with lr >= 1e-4 may be unstable; "
            "--lr 1e-5 is recommended."
        )
    ...
    summary_row = {
        "model": "samus_vit_b",
        "initialization": "official_sam_vit_b_01ec64",
        "train_scope": args.train_scope,
        "prompt_protocol": "fixed_full_image_box_no_gt",
        "preprocessing": "grayscale_resize256_tensor_0_1",
