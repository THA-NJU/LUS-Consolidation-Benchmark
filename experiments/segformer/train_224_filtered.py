...
effective batch=16, learning rate=1e-4, fixed inference threshold=0.5,
and checkpoint selection by native-224 validation Mean Dice.
"""
# Train SegFormer-B2 natively on Size_224_filtered. This file does not balance
# patches, never stitches patches to 512, and never searches validation thresholds.
...
OUTPUT_ROOT = Path("./sota_segformer_224_filtered_runs")
...
def scan_split(split: str) -> List[Tuple[Path, Path]]:
    ...
    for image in tqdm(images, desc=f"Validate filtered {split}"):
        ...
        if image_arr.shape != (IMAGE_SIZE, IMAGE_SIZE) or mask.shape != (IMAGE_SIZE, IMAGE_SIZE):
            raise ValueError(f"Expected 224x224 pair: {image}, {mask_path}")
        if not mask.any():
            raise ValueError(f"Size_224_filtered must not contain empty masks: {mask_path}")
        if mask[0].any() or mask[-1].any() or mask[:, 0].any() or mask[:, -1].any():
            raise ValueError(f"Size_224_filtered foreground touches image border: {mask_path}")
...
def evaluate(model: nn.Module, loader: DataLoader, threshold: float) -> Dict[str, float]:
    scores: List[float] = []
    model.eval()
    for images, masks, _ in tqdm(loader, desc="Native 224 validation", leave=False):
...
def main() -> None:
    set_seed()
    run_dir = OUTPUT_ROOT / f"segformer_b2_224_filtered_{datetime.now():%Y%m%d_%H%M%S}"
    run_dir.mkdir(parents=True, exist_ok=True)
    pairs = {split: scan_split(split) for split in ("train", "val", "test")}
    gray_mean, gray_std = estimate_mean_std(pairs["train"])
    train_ds = Filtered224Dataset(pairs["train"], gray_mean, gray_std, augment=True)
    val_ds = Filtered224Dataset(pairs["val"], gray_mean, gray_std, augment=False)
    test_ds = Filtered224Dataset(pairs["test"], gray_mean, gray_std, augment=False)
    ...
    config = {
        ...
        "filtered_positive_nonborder_required": True, "selection_metric": "native_224_val_mean_dice",
    ...
    best_path = run_dir / "best_segformer_b2_224_filtered.pt"
