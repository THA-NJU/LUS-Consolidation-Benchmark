#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Final native Size-512 YOLO26s semantic benchmark training script.

Protocol identity: epochs=600, patience=15, warmup=10, seed=42,
batch=4, learning rate=1e-4, fixed semantic threshold=0.5, and every epoch
ranked by native-512 validation mean per-image Dice.

This script has no positive oversampling or patch reconstruction.
"""
...
CACHE_ROOT = Path("./_yolo26_semantic_cache/Size_512")
OUTPUT_ROOT = Path("./sota_yolo26_512_runs")
...
def scan_native_split(split: str) -> List[Tuple[Path, Path]]:
    ...
    for image_path in tqdm(images, desc=f"Validate Size_512 {split}"):
        ...
        if image.shape != (IMAGE_SIZE, IMAGE_SIZE) or mask.shape != (IMAGE_SIZE, IMAGE_SIZE):
            raise ValueError(f"Expected 512x512 pair: {image_path}, {mask_path}")
        if not mask.any():
...
class Native512MeanDiceValidator(SemanticSegmentationValidator):
    ...
    def init_metrics(self, model) -> None:
        ...
        self.native_512_dice: List[float] = []
    ...
    def update_metrics(self, preds, batch) -> None:
        ...
        if torch.any(target_pixels == 0):
            raise RuntimeError("Size_512 validation unexpectedly contains an empty foreground mask")
        ...
        self.native_512_dice.extend(float(x) for x in dice.detach().cpu().tolist())

    def get_stats(self) -> Dict[str, object]:
        stats = super().get_stats()
        values = np.asarray(self.native_512_dice, dtype=np.float64)
        if values.size == 0:
            raise RuntimeError("Native 512 validation produced no per-image Dice scores")
        mean_dice = float(values.mean())
        stats["benchmark/native_512_mean_dice"] = mean_dice
        stats["benchmark/native_512_std_dice"] = float(values.std(ddof=0))
        stats["benchmark/native_512_n"] = int(values.size)
        # BaseTrainer uses and removes this key before early stopping/checkpointing.
        stats["fitness"] = mean_dice
        return stats


class Native512MeanDiceTrainer(SemanticSegmentationTrainer):
    """Use native-512 validation mean Dice as Ultralytics fitness."""

    def get_validator(self):
        return Native512MeanDiceValidator(
            self.test_loader,
...
def main() -> None:
    ...
    YOLO(str(weights))
    pairs = {split: scan_native_split(split) for split in ("train", "val", "test")}
    yaml_path = prepare_cache(pairs)
    run_dir = OUTPUT_ROOT / f"yolo26s_sem_512_{datetime.now():%Y%m%d_%H%M%S}"
    run_dir.mkdir(parents=True, exist_ok=True)
    config = {
        "model": "yolo26s_sem", "size": IMAGE_SIZE, "data_root": str(DATA_ROOT),
        "selection_metric": "native_512_val_mean_dice", "balance_train_images": False,
    ...
    model.train(
        ...
        pretrained=True, trainer=Native512MeanDiceTrainer,
    ...
    del model
    gc.collect()
    selected = run_dir / "best_by_val_mean_dice_512.pt"
    ...
    write_json(run_dir / "final_metrics.json", {
        ...
        "selection_metric": "native_512_val_mean_dice_each_epoch",
        "prediction_rule": "two-class argmax (equivalent to foreground softmax probability >= 0.5)",
